...
def create_issue(project_id, title, due_date, labels, milestone_id, private_token):
    check_milestone_id = requests.get(f'https://gitlab.com/api/v4/projects/{project_id}/milestones/{milestone_id}?private_token={private_token}')
    if check_milestone_id.status_code != 404:
        response = requests.post(f'https://gitlab.com/api/v4/projects/{project_id}/issues?title={title}&due_date={due_date}&labels={labels}&milestone_id={milestone_id}&private_token={private_token}')
    else:
        response = 'No such id'
